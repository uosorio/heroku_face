...
# Save face enconding to CSV file
def save_encoding_image(image_path, csv_name):
    encoding_dir = current_app.config['CSV_DIR']
    os.makedirs(encoding_dir, exist_ok=True)
    file_path = os.path.join(encoding_dir, csv_name)
    image = face_recognition.load_image_file(image_path)
    face_encoding = face_recognition.face_encodings(image)[0]
    logger.info(f'Se esta guardando face_encoding en la ruta: {file_path}')
    # path_file = join(current_app.config['CSV_DIR'], filename)
    savetxt(file_path, face_encoding)
